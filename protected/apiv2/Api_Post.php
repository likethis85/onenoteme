...
class Api_Post extends ApiBase
{
    ...
    public function timeline()
    {
        ...
        try {
            ...
            $condition = array('and', 'state = :enabled', 'channel_id = :channelid', 'media_type = :mediatype', 'id > :lastid');
            // @todo ios客户端现在不支持gif动画，所以先屏蔽
            if ($platform != 'android')
                $condition[] = 'original_frames <= 1';
            
            $channelMediaType = self::switchChannelMediaType($channelID);
            $param = array(':enabled'=>POST_STATE_ENABLED, ':channelid' => $channelMediaType[0], ':mediatype'=>$channelMediaType[1], ':lastid'=>$lastid);
    ...
    public function latest()
    {
        ...
        try {
            ...
            $condition = array('and', 'state = :enabled', 'channel_id = :channelid', 'media_type = :mediatype', 'create_time > :lasttime');
            // @todo ios客户端现在不支持gif动画，所以先屏蔽
            if ($platform != 'android')
                $condition[] = 'original_frames <= 1';
            
            $channelMediaType = self::switchChannelMediaType($channelID);
            $param = array(':enabled'=>POST_STATE_ENABLED, ':channelid' => $channelMediaType[0], ':mediatype'=>$channelMediaType[1], ':lasttime'=>$lasttime);
            $cmd = app()->getDb()->createCommand()
                ->select($fields)
                ->from(TABLE_POST . ' t')
                ->where($condition, $param)
...
//             var_dump($rowID);
    ...
    private static function switchChannelMediaType($channelID)
    {
        switch ($channelID)
        {
            case 0:
                $channel = CHANNEL_FUNNY;
        ...
        return array($channel, $mediaType);
    }
}
