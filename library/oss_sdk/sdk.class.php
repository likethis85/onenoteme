...
class ALIOSS{
	...
	public function auth($options){
		...
		if (isset($options['fileDownload'])){
			if (is_resource($options['fileDownload'])){
				$request->set_write_stream($options['fileDownload']);
			}else{
				$request->set_write_file($options['fileDownload']);
			}
		}
		...
		if (isset ( $options [self::OSS_CONTENT] )) {
			...
			$options[self::OSS_CONTENT_MD5] = $this->hex_to_base64(md5($options[self::OSS_CONTENT]));
		}
	...
	public function delete_objects($bucket,$objects,$options = null){
		...
		if(!is_array($objects) || !$objects){
			die('The ' . __FUNCTION__ . ' method requires the "objects" option to be set as an array.');
		}
		
		$options[self::OSS_METHOD] = self::OSS_HTTP_POST;
		$options[self::OSS_BUCKET] = $bucket;
		$options[self::OSS_OBJECT] = '/';
		$options[self::OSS_MULTI_DELETE] = 'delete';
		
		$xml = simplexml_load_string('<Delete></Delete>');
	...
	private function make_url($options) {
		...
		//Multi Delete
		if(isset($options[self::OSS_MULTI_DELETE]) && $options[self::OSS_MULTI_DELETE]){
			$url .='?delete';
		}
